...
void MainWindow::setupSpectrumPlaceholder(QWidget *parent) {
    ...
    connect(m_panadapterB, &PanadapterRhiWidget::frequencyClicked, this, [this](qint64 freq) {
        ...
        if (!m_tcpClient->isConnected() || freq <= 0)
            return;
        // L=A R=B mode: left-click on Pan B tunes VFO A (desktop mouse only).
        // Touch has no L/R button distinction, so Pan B always tunes VFO B.
#if defined(Q_OS_ANDROID) || defined(Q_OS_IOS)
    ...
    connect(m_panadapterB, &PanadapterRhiWidget::frequencyDragged, this, [this](qint64 freq) {
        ...
        if (!m_tcpClient->isConnected() || freq <= 0)
            return;
        // L=A R=B mode: left-drag on Pan B tunes VFO A (desktop mouse only).
        // Touch has no L/R button distinction, so Pan B always tunes VFO B.
#if defined(Q_OS_ANDROID) || defined(Q_OS_IOS)
        bool tuneA = false;
#else
...
void MainWindow::onAuthenticated() {
    ...
    m_tcpClient->sendCAT("#REF;#REF$;"); // Panadapter ref level (Main+Sub) - the waterfall/spectrum
                                         // dB floor; without it the floor stays at a local default and
                                         // the noise field clamps to black (only carriers show)
...
static void androidSetKeepScreenOn(bool on) {
    QNativeInterface::QAndroidApplication::runOnAndroidMainThread([on]() {
        ...
        if (!win.isValid())
            return;
        if (on)
            win.callMethod<void>("addFlags", "(I)V", 128);
        else
            win.callMethod<void>("clearFlags", "(I)V", 128);
    });
}
#endif
