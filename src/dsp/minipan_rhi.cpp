...
void MiniPanRhiWidget::render(QRhiCommandBuffer *cb) {
    ...
    if (m_waterfallNeedsUpdate && !m_smoothedSpectrum.isEmpty()) {
        ...
        for (int i = 0; i < TEXTURE_WIDTH; ++i) {
            ...
            // Match the main panadapter waterfall: lift relative to the tracked
            // noise baseline with a gain so the noise reads as visible blue
            // regardless of REF, without over-driving signals into red.
