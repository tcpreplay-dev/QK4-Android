...
void PanadapterRhiWidget::updateWaterfallData() {
    ...
    for (int i = 0; i < specSize; ++i) {
        ...
        // Android tablets often hold a REF/SCALE that leaves the noise floor near
        // the bottom of the colour range, so the waterfall reads very dim (or
        // black). Lift the value relative to the tracked noise baseline (same
        // self-normalising trick the spectrum trace uses) and apply a gain so the
        // noise field always shows as visible blue regardless of REF. macOS/iOS
        // keep the raw REF-based mapping.
        constexpr float kWaterfallNoiseGain = 1.7f;
        float lifted = qMax(0.0f, normalized - m_smoothedBaseline) * kWaterfallNoiseGain;
        // Small ambient floor so the quietest bins still read as faint blue
        // rather than pure black, matching the macOS look.
        lifted = qMax(lifted, normalized * 0.08f);
        normalized = qBound(0.0f, lifted, 1.0f);
#endif
